...
const removeIgnore = (value) => {
  let str = value;

  ignoreCustomFragments.forEach((item) => {
    str = str.replace(item, (match) => {
...
const loadDeps = () => {
  ...
  if (!HTMLHint) {
    ({ default: HTMLHint } = require('htmlhint'));
  }
...
export default {
  activate() {
    ...
    const installLinterHtmlhintDeps = () => {
      ...
      if (!atom.inSpecMode()) {
        require('atom-package-deps').install('linter-htmlhint-plus');
      }
    ...
    this.subscriptions.add(atom.config.observe('linter-htmlhint-plus.enabledScopes', (scopes) => {
    ...
    this.subscriptions.add(atom.config.observe('linter-htmlhint-plus.disableWhenNoHtmlhintConfig', (value) => {
      disableWhenNoHtmlhintConfig = value;
    }));

    this.subscriptions.add(atom.config.observe('linter-htmlhint-plus.ignoreCustomFragments', (values) => {
      const result = [];

      values.forEach((item) => {
        const match = item.match(/^\/(.*?)\/([gimy]*)$/);

        if (match) {
          result.push(new RegExp(match[1], `${match[2].replace(/g/g, '')}g`));
        }
      });

      ignoreCustomFragments = result;
    }));
  },
  ...
  provideLinter() {
    return {
      ...
      lint: async (editor) => {
        ...
        const text = removeIgnore(fileText);
        if (!text) {
          return [];
        }
